...
@section('content')
    <header class="common-header">
        <div class="header-wrapper">
            <h1 class="title-large">Inserir nova categoria </h1>
        </div>
        <div class="pattern__header"></div>
    </header>
        <section class="contact-wrapper">
            <form class="form-group" action="{{route('site.categories.add')}}" method="post" style="margin-bottom: 30px;" enctype="multipart/form-data">
                    @csrf
                    @if(session('success'))
                        <div>
                            {{session('message')}}
                    ...
                    <input id="image" name="image" type="file" tabindex="1" placeholder=""
                           autofocus value="{{old('image')}}" accept="image/png, image/jpeg"  class="form-control">
                    @error('image')
                    <div>{{ $message }}</div>
                    @enderror
